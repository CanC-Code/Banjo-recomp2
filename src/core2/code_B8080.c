...
BKModel *func_8033F5F8(BKMeshList *meshList, BKVertexList *vertexList) {
    ...
    sp40 = (BKModel *)n64_malloc((meshList_getVtxCount(meshList) * sizeof(BKVtxRef)) + (meshList->meshCount_0 * sizeof(BKMesh)) + sizeof(BKModel));
    ...
    for(phi_s6 = 0; phi_s6 < meshList->meshCount_0; phi_s6++){
            ...
            for(phi_s1 = 0; phi_s1 < phi_s3->vtxCount_2; phi_s1++){
                ...
                n64_memcpy(phi_s0, ((Vtx *)(vertexList + 1)) + phi_s0->unk10, sizeof(Vtx));
                phi_s0++;
            }
